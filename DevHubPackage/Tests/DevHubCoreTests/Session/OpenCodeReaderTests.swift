...
struct OpenCodeReaderTests {
    ...
    func discoversMetadata() async throws {
        ...
        #expect(sessions[0].messageCount == 0)
    ...
    @Test("conversation bodies remain an explicit unsupported capability")
    func loadIsExplicitlyUnavailable() async {
        let reader = OpenCodeReader(databaseURLs: [])
        await #expect(throws: OpenCodeReaderError.conversationUnavailable("ses_1")) {
            _ = try await reader.load("ses_1")
        }
    }
