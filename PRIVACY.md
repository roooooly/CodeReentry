...
DevHub is a local resource manager. Its purpose requires access to files and metadata
that may be private, so the boundary should be explicit.

## Data DevHub can read

Depending on enabled features and user configuration, DevHub can read:
...
- subscription and platform-account records entered in DevHub.
...
DevHub writes its SwiftData store, preferences, indexes, and caches to the current macOS
...
DevHub has no telemetry or analytics service. Network access can still occur when the
user explicitly:
...
Diagnostic export is user initiated. It is limited to recent entries from DevHub's own
